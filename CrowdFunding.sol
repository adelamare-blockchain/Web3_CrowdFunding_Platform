...
contract CrowdFunding { 
    ...
    // Fonction qui permet de faire un don à une campagne de crowdfunding + modifier nonReentrant
    function donateToCampaign(uint256 _id) public payable {
        ...
        // Check si la target de campagne n'est pas dépassé
        require(campaign.amountCollected + amount <= campaign.target, "Campaign target has been reached."); 

        // Implémentation de la donation dans l'array
        campaign.donators.push(msg.sender);
        campaign.donations.push(amount);

        // Ajout de l'amount à la target de campaign
        campaign.amountCollected += amount;

        // Envoi des fonds au créateur de la campagne
        require(payable(campaign.owner).send(amount),"Failed to send funds");
    }
